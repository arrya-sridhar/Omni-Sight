...
class MockDatabase(Database):
        ...
        self.keyframes = [
            {
                ...
                "timestamp": 2.0,
                "embedding": [
                    1.0 if i == 0 else 0.0 for i in range(512)
                ],  # matches "person"
            ...
            },
            {
                ...
                "timestamp": 4.0,
                "embedding": [
                    0.0 if i == 0 else 1.0 for i in range(512)
                ],  # matches other
    ...
    def save_tracked_objects(self, tracked_objects):
        pass
...
def test_similarity_search_ranking():
    ...
    results = service.search(
        query="a person walking", video_ids=["vid-1"], threshold=0.5, limit=5
    )
...
def test_similarity_search_no_matches():
    ...
    results = service.search(
        query="person", video_ids=["vid-1"], threshold=0.99, limit=5
    )
    # The matching keyframe (kf-1) has score 1.0, so it passes. kf-2 has score 0.0, so it fails.
    assert len(results) == 1

    results_high = service.search(
        query="something else", video_ids=["vid-1"], threshold=0.99, limit=5
    )
