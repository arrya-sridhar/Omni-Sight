...
def test_keyframe_extraction_similarity_filter():
    ...
    is_kf_1, diff_1 = extractor.process_frame(
        frame_black_1, frame_index=0, timestamp=0.0
    )
    assert is_kf_1 is True

    # Process Frame 2 (identical to Frame 1, should be filtered)
    is_kf_2, diff_2 = extractor.process_frame(
        frame_black_2, frame_index=1, timestamp=0.1
    )
