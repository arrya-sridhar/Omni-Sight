...
async def log_requests(request: Request, call_next):
    ...
    try:
        ...
        logger.info(
            f"Request completed: {request.method} {request.url.path} - Status: {response.status_code} - In {process_time:.2f}ms"
        )
        return response
    except Exception as exc:
        process_time = (time.time() - start_time) * 1000
        logger.error(
            f"Request failed: {request.method} {request.url.path} - Exception: {str(exc)} - In {process_time:.2f}ms"
        )
        raise exc
...
async def global_exception_handler(request: Request, exc: Exception):
    logger.error(
        f"Unhandled system error on {request.url.path}: {str(exc)}", exc_info=True
    )
    return JSONResponse(
...
async def health_check():
    return {"status": "healthy", "timestamp": time.time(), "service": "omnisight-ai"}
